package com.mrdongshan.camunda.service.impl;
...
import com.mrdongshan.camunda.handler.IUserTaskExecuteHandler;
import com.mrdongshan.camunda.service.IBpmnService;
import com.mrdongshan.camunda.util.ParamAssert;
...
public class BpmnServiceImpl implements IBpmnService {
    ...
    public List<TaskBo> finishTask(ProcessFormVo vo) {
        ...
        List<IUserTaskExecuteHandler> executeHandlers = iUserTaskExecuteHandlers.stream().filter(v -> v.match(vo.getTaskCode())).collect(Collectors.toList());
    ...
    public void terminateProcess(ProcessFormVo vo) {
        ...
        List<IUserProcessExecuteHandler> executeHandlers = iUserProcessExecuteHandlers.stream().filter(v -> v.match(key)).collect(Collectors.toList());
    ...
    public List<TaskBo> start(String processKey, Map<String, Object> params) {
        ...
        //业务执行逻辑
        List<IUserProcessExecuteHandler> processExecuteHandlers = iUserProcessExecuteHandlers.stream().filter(v -> v.match(processKey)).collect(Collectors.toList());
